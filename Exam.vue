<template>
    <div class="page-card">
        <div class="page-title">开立检查申请</div>
        ...
        <template v-if="patient">
            <el-divider content-position="left">已开检查申请</el-divider>
            ...
            <el-divider content-position="left">新建检查申请</el-divider>
            <el-form :model="form" label-width="100px" size="small" inline>
                ...
                <el-form-item>
                    <el-button type="primary" @click="submit">开立检查申请</el-button>
                </el-form-item>
            </el-form>
        </template>
    </div>
...
<script>
import { getRegistrationByRegNo, getExamItemList, addExamApply, getExamApplyList, getRegistrationByDoctor } from '../../api'
...
export default {
    ...
    methods: {
        loadItems() { getExamItemList().then(res => { this.itemList = res.data || [] }) },
        loadPatientList() {
        ...
        loadApply() {
            getExamApplyList({ patientId: this.patient.patientId }).then(res => { this.applyList = res.data || [] })
        },
        submit() {
            if (!this.form.itemId) { this.$message.warning('请选择检查项目'); return }
            const user = JSON.parse(localStorage.getItem('user'))
            addExamApply({
                ...
                this.$message.success('检查申请开立成功')
                this.form = { itemId: null, clinicalInfo: '' }
                this.loadApply()
            })
        },
        statusText(s) { return { UNPAID: '未缴费', PAID: '已缴费', EXECUTING: '检查中', COMPLETED: '已完成' }[s] || s },
