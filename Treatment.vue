<template>
    <div class="page-card">
        <div class="page-title">开立处置申请</div>
        ...
        <template v-if="patient">
            <el-divider content-position="left">已开处置申请</el-divider>
            ...
            <el-divider content-position="left">新建处置申请</el-divider>
            <el-form :model="form" label-width="100px" size="small" inline>
                ...
                <el-form-item>
                    <el-button type="primary" @click="submit">开立处置申请</el-button>
                </el-form-item>
            </el-form>
        </template>
    </div>
...
<script>
import { getRegistrationByRegNo, getTreatmentItemList, addTreatmentApply, getTreatmentApplyList, getRegistrationByDoctor } from '../../api'
...
export default {
    ...
    methods: {
        loadItems() { getTreatmentItemList().then(res => { this.itemList = res.data || [] }) },
        loadPatientList() {
        ...
        loadApply() {
            getTreatmentApplyList({ patientId: this.patient.patientId }).then(res => { this.applyList = res.data || [] })
        },
        submit() {
            if (!this.form.itemId) { this.$message.warning('请选择处置项目'); return }
            const user = JSON.parse(localStorage.getItem('user'))
            addTreatmentApply({
